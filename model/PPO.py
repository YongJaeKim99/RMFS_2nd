...
class Memory:
    def __init__(self, gamma, gae_lambda):
        ...
        self.fea_act_seq = []  # [N, tensor[sz_b, N, 8]]
        self.act_mask_seq = []  # [N, tensor[sz_b, N, 3]]
        self.fea_team_seq = []  # [N, tensor[sz_b, T, 6]]
        self.team_mask_seq = []  # [N, tensor[sz_b, T, T]]
        self.dynamic_pair_mask_seq = []  # [N, tensor[sz_b, P, T]]
        self.comp_idx_seq = []  # [N, tensor[sz_b, T, T, P]]
        self.candidate_seq = []  # [N, tensor[sz_b, P]]
        self.fea_pairs_seq = []  # [N, tensor[sz_b, P]]
    ...
    def transpose_data(self):
        ...
        """
        # 14
        t_Fea_act_seq = torch.stack(self.fea_act_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_act_mask_seq = torch.stack(self.act_mask_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_Fea_team_seq = torch.stack(self.fea_team_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_team_mask_seq = torch.stack(self.team_mask_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_dynamicMask_seq = torch.stack(self.dynamic_pair_mask_seq, dim=0).transpose(0, 1).flatten(0, 1)
        t_Compete_team_seq = torch.stack(self.comp_idx_seq, dim=0).transpose(0, 1).flatten(0, 1)
        ...
        return t_Fea_act_seq, t_act_mask_seq, t_Fea_team_seq, t_team_mask_seq, t_dynamicMask_seq, \
               t_Compete_team_seq, t_candidate_seq, t_pairMessage_seq, \
    ...
    def update(self, memory):
        ...
        for _ in range(self.k_epochs):
            ...
            for i in range(int(num_batch)):
                ...
                pis, vals = self.policy(fea_act=t_data[0][start_idx:end_idx],
                                        act_mask=t_data[1][start_idx:end_idx],
                                        candidate=t_data[6][start_idx:end_idx],
                                        fea_team=t_data[2][start_idx:end_idx],
                                        team_mask=t_data[3][start_idx:end_idx],
