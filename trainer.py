...
from model.gat_model import GATSchedulingModel
from model.topoformer_model import TopoformerSchedulingModel
...
class Scheduling_Trainer:
    def __init__(self,
        ...
        if mode == 'train':
            ...
            model_type_label = trainer_params.get('model_type', 'daniel').upper().replace('_', '')
            alg_type = trainer_params.get('algorithm_type', 'reinforce')
            alg_label = {'ppo': 'PPO', 'il': 'IL', 'reinforce': 'REINFORCE'}.get(alg_type, 'REINFORCE')
            self.checkpoint_dir = f"./checkpoints/{timestamp}_{objective}_{model_type_label}_{alg_label}"
        ...
        # 모델 타입 설정
        self.model_type = trainer_params.get('model_type', 'daniel')
        ...
        if self.model_type == 'daniel':
            ...
            daniel_config = SimpleNamespace(
                ...
                fea_j_input_dim=self.model_params['fea_act_input_dim'],
                fea_m_input_dim=self.model_params['fea_team_input_dim'],
                fea_pairs_input_dim=self.model_params.get('fea_pairs_input_dim', 3),
                num_heads_OAB=self.model_params['num_heads_AAB'],
                num_heads_MAB=self.model_params['num_heads_TAB'],
        ...
        elif self.model_type in ('gat_v1', 'gat_v2', 'rgat'):
            # GAT 모델 초기화
            gat_version_map = {'gat_v1': 'v1', 'gat_v2': 'v2', 'rgat': 'relational'}
            gat_config = SimpleNamespace(
                fea_act_input_dim=self.model_params['fea_act_input_dim'],
                fea_team_input_dim=self.model_params['fea_team_input_dim'],
                edge_feat_dim=self.model_params.get('edge_feat_dim', 10),
                d_hidden=self.model_params['d_hidden'],
                n_gat_layers=self.model_params['n_gat_layers'],
                n_heads=self.model_params['n_heads'],
                n_edge_types=self.model_params.get('n_edge_types', 4),
                gat_version=gat_version_map[self.model_type],
                dropout_prob=self.model_params['dropout_prob'],
                num_mlp_layers_actor=self.model_params['num_mlp_layers_actor'],
                hidden_dim_actor=self.model_params['hidden_dim_actor'],
                num_mlp_layers_critic=self.model_params['num_mlp_layers_critic'],
            ...
            self.model = GATSchedulingModel(gat_config).to(self.device)
        elif self.model_type == 'topoformer':
            # Topoformer 모델 초기화
            from types import SimpleNamespace as _NS
            topoformer_config = _NS(
                fea_act_input_dim=self.model_params['fea_act_input_dim'],
                fea_team_input_dim=self.model_params['fea_team_input_dim'],
                edge_feat_dim=self.model_params.get('edge_feat_dim', 10),
                d_hidden=self.model_params['d_hidden'],
                n_topoformer_layers=self.model_params['n_topoformer_layers'],
                heads_per_mask=self.model_params['heads_per_mask'],
                mlp_ratio=self.model_params.get('mlp_ratio', 4),
                dropout_prob=self.model_params['dropout_prob'],
                num_mlp_layers_actor=self.model_params['num_mlp_layers_actor'],
                hidden_dim_actor=self.model_params['hidden_dim_actor'],
                num_mlp_layers_critic=self.model_params['num_mlp_layers_critic'],
            ...
            self.model = TopoformerSchedulingModel(topoformer_config).to(self.device)
        else:
        ...
        # 환경 state_mode 설정
        if self.model_type == 'topoformer':
            self.env_state_mode = 'topoformer'
        elif self.model_type in ('gat_v1', 'gat_v2', 'rgat'):
            self.env_state_mode = 'gat'
        else:
            self.env_state_mode = 'daniel'

        model_label = self.model_type.upper().replace('_', '')
        print(f"✅ {model_label} 모델 초기화 완료")
        ...
        if self.algorithm_type == 'ppo':
            ...
            self.ppo_memory         = PPOMemory(self.gamma, self.gae_lambda, model_type=self.model_type)
            print(f"✅ PPO 초기화 완료 (eps_clip={self.eps_clip}, k_epochs={self.k_epochs}, "
                  f"gae_lambda={self.gae_lambda}, n_resample={self.n_resample})")

        # IL (Imitation Learning) 전용 초기화 (학습 모드에서만)
        elif self.algorithm_type == 'il' and mode == 'train':
            ...
            self.il_label_smoothing = trainer_params.get('il_label_smoothing', 0.0)
            il_data_path = trainer_params.get('il_data_path')
            if not il_data_path:
                # 자동 탐색: data/il/il_trajectory/ 폴더 (디렉토리 전체 로드)
                from pathlib import Path
                traj_dir = Path('data/il/il_trajectory')
                if traj_dir.exists():
                    candidates = sorted(traj_dir.glob('*_trajectory*.pickle'))
                    if candidates:
                        # 디렉토리 경로를 전달 → ILDataset이 전체 파일을 합쳐서 shuffle
                        il_data_path = str(traj_dir)
                        print(f"  IL trajectory 디렉토리 자동 탐색: {il_data_path} ({len(candidates)}개 파일)")
                if not il_data_path:
                    raise ValueError(
                        "IL trajectory 파일을 찾을 수 없습니다.\n"
                        "  1) il_trajectory_generation.py로 trajectory를 먼저 생성하거나\n"
                        "  2) train.py의 IL_DATA_PATH에 경로를 직접 지정해주세요."
                    )
            self.il_dataset = ILDataset(il_data_path)

            # Trajectory의 state_mode와 현재 MODEL_TYPE 불일치 검증
            if self.il_dataset.config:
                traj_state_mode = self.il_dataset.config.get('state_mode')
                current_state_mode = self.env_params.get('state_mode')
                if traj_state_mode and current_state_mode and traj_state_mode != current_state_mode:
                    raise ValueError(
                        f"IL trajectory state_mode 불일치!\n"
                        f"  trajectory: state_mode='{traj_state_mode}' "
                        f"(model_type='{self.il_dataset.config.get('model_type')}')\n"
                        f"  train.py:   state_mode='{current_state_mode}' "
                        f"(MODEL_TYPE='{self.model_type}')\n"
                        f"  → train.py의 MODEL_TYPE을 trajectory 생성 시와 동일하게 맞춰주세요."
                    )
            ...
            print(f"✅ IL 초기화 완료 (loss_type={self.il_loss_type}, "
                  f"label_smoothing={self.il_label_smoothing}, "
           ...
           #wandb.login(key='')
            ...
            if self.algorithm_type == 'il':
                config.update({
                    ...
                    "il_label_smoothing": self.trainer_params.get('il_label_smoothing', 0.0),
        ...
        try:
          for epoch in range(start_epoch, end_epoch+1):
            ...
            if self.algorithm_type == 'ppo':
                ...
                train_loss_avg, train_reward_avg, train_obj_avg = self._train_ppo_one_batch(ppo_problem)
            elif self.algorithm_type == 'il':
                train_loss_avg = self._train_il_one_epoch()
                train_obj_avg = None
                train_reward_avg = None
            else:
                train_loss_avg, train_reward_avg, train_obj_avg = self._train_one_batch()
            epoch_elapsed_time = time.time() - epoch_start_time

            if self.algorithm_type == 'il':
                print(f"epoch: {epoch}, loss: {train_loss_avg:.4f}, time: {epoch_elapsed_time:.2f}s")
            else:
            ...
            # Train score 업데이트
            if train_obj_avg is not None and train_obj_avg < best_train_score:
            ...
            if train_obj_avg is not None:
                wandb_log_dict["train/" + objective_name] = train_obj_avg
                wandb_log_dict["train/best_" + objective_name] = best_train_score
    ...
    def _train_one_batch(self):
        ...
        total_reward = 0  # 전체 보상 초기화 (정규화 스케일)
        total_obj = 0  # 전체 목적함수 누적 (원본 스케일, 로깅용)

        for i in range(self.trainer_params['accumulation_steps']):
            if self.verbose_logging:
                print(f"accumulation_steps: {i}")

            loss, obj_value, obj_normalized, step_count, batch_info = self.train_one_minibatch()
            ...
            total_reward += -obj_normalized  # 보상 누적 (정규화 스케일: reward = -normalized_obj)
            total_obj += obj_value  # 원본 스케일 tardiness 누적 (로깅용)
        ...
        n_acc = self.trainer_params['accumulation_steps']
        avg_loss = total_loss / n_acc
        avg_reward = total_reward / n_acc
        avg_obj = total_obj / n_acc

        return avg_loss, avg_reward, avg_obj

    # =================================================================
    # Model Forward Helper (DANIEL / GAT 공통)
    # =================================================================

    def _model_forward(self, s):
        """
        State 객체에서 텐서를 추출하고 모델 forward를 호출.
        DANIEL(EnvState)과 GAT(GATState) 모두 지원.
        """
        if self.model_type == 'daniel':
            ...
            fea_act     = s.fea_act_tensor.to(self.device)
            fea_team    = s.fea_team_tensor.to(self.device)
            topo_masks  = s.topo_masks_tensor.to(self.device)
            edge_feat   = s.edge_feat_tensor.to(self.device)
            dyn_pmask   = s.dynamic_pair_mask_tensor.to(self.device)
            node_valid  = s.node_valid_mask_tensor.to(self.device)
            return self.model(
                fea_act, fea_team, topo_masks, edge_feat, dyn_pmask, node_valid
            )
        else:  # gat_v1, gat_v2, rgat
            fea_act    = s.fea_act_tensor.to(self.device)
            fea_team   = s.fea_team_tensor.to(self.device)
            adj        = s.adj_tensor.to(self.device)
            edge_feat  = s.edge_feat_tensor.to(self.device)
            dyn_pmask  = s.dynamic_pair_mask_tensor.to(self.device)
            node_valid = s.node_valid_mask_tensor.to(self.device)
            return self.model(
                fea_act, fea_team, adj, edge_feat, dyn_pmask, node_valid
            )
    ...
    def _train_ppo_one_batch(self, problem):
        ...
        env.state_mode = self.env_state_mode
        env._reset(problem)
        ...
        final_obj = env._get_obj_original_scale().mean().item()  # 원본 스케일 (로깅용)
        final_obj_normalized = env._get_obj().mean().item()      # 정규화 스케일 (reward용)
        ...
        for _ in range(self.k_epochs):
            for i in range(num_mini):
                ...
                if self.model_type == 'daniel':
                    ...
                    pi_new, v_new = self.model(
                        t_data[0][start:end],   # fea_act
                        t_data[1][start:end],   # fea_team
                        t_data[2][start:end],   # topo_masks
                        t_data[3][start:end],   # edge_feat
                        t_data[4][start:end],   # dynamic_pair_mask
                        t_data[5][start:end],   # node_valid_mask
                    )
                else:  # gat_v1, gat_v2, rgat
                    pi_new, v_new = self.model(
                        t_data[0][start:end],   # fea_act
                        t_data[1][start:end],   # fea_team
                        t_data[2][start:end],   # adj
                        t_data[3][start:end],   # edge_feat
                        t_data[4][start:end],   # dynamic_pair_mask
                        t_data[5][start:end],   # node_valid_mask
                    )

                actions_batch   = t_data[-5][start:end]  # action
                old_logprobs    = t_data[-1][start:end]   # stored log_probs
        ...
        avg_reward = -final_obj_normalized  # 정규화 스케일 reward
        avg_obj    = final_obj              # 원본 스케일 tardiness (로깅용)
    ...
    def train_one_minibatch(self):
        ...
        # 목적함수값 계산
        obj_values = env._get_obj_original_scale()  # (batch_size * pomo_size,) 원본 스케일 (로깅용)
        obj_normalized = env._get_obj().mean().item()  # 정규화 스케일 (reward용)
        ...
        if self.verbose_logging:
            ...
            for b_idx in range(self.env_params['batch_size']):
                ...
                for p_idx in range(self.env_params['pomo_size']):
                    ...
                    num_started = env.activity_reserved[instance_idx, :num_total_activities].sum().item()
        ...
        return loss, obj_value, obj_normalized, step_count, batch_info
    ...
    def _prepare_validation_dataset(self, validation_batch_size=10, validation_pomo_size=1):
        ...
        # 파일명: val_P{N_P}_A{N_A}_T{N_T}_d{tightness}_{instances}instances.pickle
        _ep = self.env_params
        _N_A = _ep.get('N_A_max', _ep.get('N_A_min', '?'))
        _N_T = _ep.get('N_T', '?')
        _dd = _ep.get('due_date_tightness', '?')
        _N_P = _ep.get('N_P', '?')
        val_filename = f"val_P{_N_P}_A{_N_A}_T{_N_T}_d{_dd}_{validation_batch_size}instances.pickle"
        batch_file_path = os.path.join(val_data_folder, val_filename)
    ...
    def _train_il_one_epoch(self):
        ...
        for batched_state, actions in self.il_dataloader:
            ...
            # Label smoothing + CE 시 logits 필요, 그 외는 pi만 사용
            use_logits = (self.il_loss_type == 'ce' and self.il_label_smoothing > 0)

            # Model forward: 데이터의 키로 state 형식 자동 감지
            # (model_type과 데이터 불일치 방지)
            is_topoformer_data = 'topo_masks' in batched_state
            is_gat_data = 'adj' in batched_state
            if is_topoformer_data:
                # Topoformer state keys
                fea_act = batched_state['fea_act'].to(self.device)
                fea_team = batched_state['fea_team'].to(self.device)
                topo_masks = batched_state['topo_masks'].to(self.device)
                edge_feat = batched_state['edge_feat'].to(self.device)
                dynamic_pair_mask = batched_state['dynamic_pair_mask'].to(self.device)
                node_valid_mask = batched_state['node_valid_mask'].to(self.device)
                args = (fea_act, fea_team, topo_masks, edge_feat,
                        dynamic_pair_mask, node_valid_mask)
            elif not is_gat_data:
                # DANIEL state keys
                fea_act = batched_state['fea_act'].to(self.device)
                ...
                mutex_idx = batched_state['mutex_idx'].to(self.device) if batched_state.get('mutex_idx') is not None else None
                args = (fea_act, act_mask, candidate, fea_team,
                        team_mask, comp_idx, dynamic_pair_mask, fea_pairs,
                        pred_idx, succ_idx, mutex_idx)
            else:
                # GAT state keys (gat_v1, gat_v2, rgat)
                fea_act = batched_state['fea_act'].to(self.device)
                fea_team = batched_state['fea_team'].to(self.device)
                adj = batched_state['adj'].to(self.device)
                edge_feat = batched_state['edge_feat'].to(self.device)
                dynamic_pair_mask = batched_state['dynamic_pair_mask'].to(self.device)
                node_valid_mask = batched_state['node_valid_mask'].to(self.device)
                args = (fea_act, fea_team, adj, edge_feat,
                        dynamic_pair_mask, node_valid_mask)

            if use_logits:
                pi, v, logits = self.model(*args, return_logits=True)
            else:
                pi, v = self.model(*args)
            ...
            elif self.il_label_smoothing > 0:
                # CE + Label Smoothing: masked(-inf) 위치를 매우 작은 값으로 대체
                # -inf가 남아 있으면 label smoothing이 해당 클래스에 확률 배분 시 loss 폭발
                logits = logits.clamp(min=-100)
                loss = F.cross_entropy(logits, optimal_actions,
                                       label_smoothing=self.il_label_smoothing)
            else:
    ...
    def _eval_validation(self, validation_batch_size=10, validation_pomo_size=1):
        ...
        if self.validation_problem is not None:
            ...
            # 파라미터 기반 파일명으로 탐색, 없으면 폴더 내 첫 번째 pickle 사용
            from pathlib import Path as _Path
            val_dir = _Path('./data/val')
            _ep = self.env_params
            _N_A = _ep.get('N_A_max', _ep.get('N_A_min', '?'))
            _N_T = _ep.get('N_T', '?')
            _dd = _ep.get('due_date_tightness', '?')
            _N_P = _ep.get('N_P', '?')
            val_filename = f"val_P{_N_P}_A{_N_A}_T{_N_T}_d{_dd}_{validation_batch_size}instances.pickle"
            batch_file_path = val_dir / val_filename
            if not batch_file_path.exists():
                # 하위 호환: 기존 val_batch.pickle 또는 폴더 내 첫 pickle
                batch_file_path = val_dir / 'val_batch.pickle'
            if not batch_file_path.exists():
                candidates = sorted(val_dir.glob('*.pickle'))
                if candidates:
                    batch_file_path = candidates[0]
            with open(batch_file_path, 'rb') as f:
        ...
        val_env.state_mode = self.env_state_mode
