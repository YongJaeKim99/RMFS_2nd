...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------
    # Company/Set 계층 구조 설정
    # ------------------------------------------------------------------
    USE_HIERARCHY = False  # True: Company/Set 계층 구조, False: 기존 flat 모델

    # Hierarchy 모드 파라미터 (USE_HIERARCHY=True일 때만 사용)
    SET_SIZES = [9, 9, 1, 2, 9, 1]            # Set별 activity 수 (N_A = sum = 31)
    N_C = 4                                     # Company 수
    TEAMS_PER_COMPANY = [3, 5, 3, 6]           # Company별 팀 수 (N_T = sum = 17)
    SET_TO_COMPANIES = {0: [0], 1: [1, 2], 2: [3], 3: [1, 2], 4: [1, 2], 5: [3]}
    COMPANY_ELIGIBLE_RATIO = 0.8               # Company 내 eligible 비율

    env_params = {
        'batch_size': NUM_INSTANCES,
        'pomo_size': POMO_SIZE,
        'N_P': 3,  # 프로젝트 수
        'N_A_min': 30,  # 프로젝트당 최소 activity 수
        'N_A_max': 30,  # 프로젝트당 최대 activity 수
        'N_T': 10,  # 팀 수 (flat 모드에서 사용, hierarchy 모드에서는 자동 산출)
        'duration_min': 3,  # 최소 작업 시간
        'duration_max': 10,  # 최대 작업 시간
        'precedence_prob': 0.3,  # 선행 관계 생성 확률
        'mutex_prob': 0.03,  # 동시 불가 생성 확률
        'max_preds': 6,   # activity당 최대 선행 작업 수 (tensor 패딩 크기)
        'max_succs': 6,   # activity당 최대 후행 작업 수 (tensor 패딩 크기)
        'max_mutex': 3,  # activity당 최대 동시 불가 작업 수 (tensor 패딩 크기)
        'eligible_teams_ratio': 0.6,  # 평균 eligible 팀 비율 (flat 모드)
        'due_date_tightness': 1.5,  # Due date 여유도 (1.0 = tight, 1.5 = loose)
        'objective': OBJECTIVE,
        'debug_env': False,
    }
    ...
    if USE_HIERARCHY:
        env_params.update({
            ...
            'set_sizes': SET_SIZES,
            'N_C': N_C,
            'teams_per_company': TEAMS_PER_COMPANY,
            'set_to_companies': SET_TO_COMPANIES,
            'company_eligible_ratio': COMPANY_ELIGIBLE_RATIO,
            'N_A_min': sum(SET_SIZES),
            'N_A_max': sum(SET_SIZES),
        })
