...
RCMPSP (3-slot, use_mutex_attention=False):
        act_mask[act, 0] = DAG source (선행자 없음) → pred_agg 스킵
...
RCMPSP (4-slot, use_mutex_attention=True):
        act_mask[act, 0] = DAG source (선행자 없음) → pred_agg 스킵
        act_mask[act, 2] = DAG sink (후행자 없음) → succ_agg 스킵
        act_mask[act, 3] = mutex 파트너 없음 → mutex_agg 스킵
        → 패딩 activity는 전부 mask (1.0)
```
...
class EnvState:
    ...
    act_mask_tensor:          Tensor  # (B, N, 3 or 4) — activity attention mask (use_mutex_attention 시 4)
    ...
    mutex_idx_tensor:         Tensor  # (B, N, max_mutex) — mutex partner 인덱스 (use_mutex_attention 시, 아니면 None)
```
...
5. ~team_avail_now:     팀이 현재 바쁨 (move_next_state가 팀이 풀릴 때까지 시간 진행)

조건부 마스킹:
6. ~act_released:       release time 미도래  (allow_wait_release=False일 때만)
7. ~mutex_ok:           mutex 파트너 실행 중  (allow_wait_mutex=False일 때만)

Dominance rule (allow_wait + dominance_rule=True일 때):
8. dominated:           대기 idle time 동안 다른 즉시 실행 가능 activity를 할 수 있는 pair
```
